  ...
  return (
    <TouchableOpacity style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }} onPress={onPress}>
      <View style={[styles.shadow, { width: 60, height: 60 }]}>
        <LinearGradient
          style={[
            { flex: 1, alignItems: 'center', justifyContent: 'center', borderRadius: 15, backgroundColor: 'red' },
          ]}
          colors={bgColor}
          start={{ x: 0, y: 0 }}
          end={{ x: 0, y: 1 }}
...
const Home = ({ navigation }) => {
  ...
  function renderCourses(item, index) {
    var coursesStyle = {};

    if (index == 0) {
      coursesStyle = { marginLeft: SIZES.padding };
    }

    return (
      <TouchableOpacity
        style={{ justifyContent: 'center', marginHorizontal: SIZES.base, ...coursesStyle }}
        onPress={() => {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Courses */}
      <View style={{ flex: 1 }}>
        <Text style={{ marginTop: SIZES.base, marginHorizontal: SIZES.padding, ...FONTS.h2 }}>Cursos</Text>
        <FlatList
          horizontal
